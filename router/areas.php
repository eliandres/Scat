...
switch ($_SERVER['REQUEST_METHOD']) {
    case 'GET':
        ...
        if (isset($_GET['id'])) {
            echo json_encode(Areas::obtenerId($_GET['id']));
        } //end if
        else {
            echo json_encode(Areas::lista());
        } //end else
        break;
    case 'POST':
        $tokenCode = Seguridad::validaTokenJwt();
        ...
        if ($datos != NULL) {
            if (Areas::insertar($datos->nombre, $datos->ap, $datos->am, $datos->fn, $datos->genero)) {
    ...
    case 'PUT':
        ...
        $datos = json_decode(file_get_contents('php://input'));
        if ($datos != NULL) {
            if (Areas::actualizar($datos->id, $datos->nombre, $datos->ap, $datos->am, $datos->fn, $datos->genero)) {
